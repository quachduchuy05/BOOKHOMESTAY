    ...
    public BookingSuggestionResponse processChat(BookingChatRequest request) {
        ...
        if (!hasLocation) {
            if (newFilters.getExplanation() != null && !newFilters.getExplanation().isBlank()) {
                ...
                response.setSystemPrompt("Dạ, bạn muốn tìm homestay hoặc villa ở khu vực nào (ví dụ: Đà Lạt, Sa Pa, Vũng Tàu, Hà Nội...)? Cho mình biết để tìm ngay nhé!");
            }
            return response;
        }

        // Nếu khách chưa nói số người, mặc định gợi ý cho 2 người (tiêu chuẩn) để khách xem phòng ngay
        if (currentFilters.getGuestCount() == null || currentFilters.getGuestCount() < 1) {
            currentFilters.setGuestCount(2);
        }
        
        // Mặc định số lượng phòng là 1 nếu chưa chọn
        if (currentFilters.getRoomCount() == null || currentFilters.getRoomCount() < 1) {
        ...
        if (foundRooms.isEmpty()) {
            ...
            List<RoomSuggestionDto> dtos = foundRooms.stream().map(room -> {
                ...
                String imgUrl = (room.getHomestay().getImages() != null && !room.getHomestay().getImages().isEmpty())
                        ? room.getHomestay().getImages().get(0).getImageUrl()
